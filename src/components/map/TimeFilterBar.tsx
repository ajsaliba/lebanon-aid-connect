...
const REGION_PRESETS = [
  ...
  { id: 'latam', label: 'LatAm', center: [-10, -60] as [number, number], zoom: 3 },
...
export function TimeFilterBar({ activeTime, onTimeChange }: TimeFilterBarProps) {
  ...
  return (
    <div className="absolute top-3 right-3 z-[1000] flex flex-col items-end gap-1 max-w-[calc(100vw-70px)]">
      {/* Time filters */}
      <div className="bg-card/90 border border-border backdrop-blur-sm rounded-md px-1 py-0.5 flex items-center gap-0 overflow-x-auto scrollbar-none">
        <Clock className="h-3 w-3 text-muted-foreground mx-1 shrink-0" />
        {TIME_RANGES.map(t => (
          <button
            key={t.id}
            onClick={() => onTimeChange(t.id)}
            className={cn(
              'px-1.5 py-0.5 rounded text-[9px] font-bold uppercase transition-colors shrink-0',
              activeTime === t.id
      ...
      {/* Region presets */}
      <div className="bg-card/90 border border-border backdrop-blur-sm rounded-md px-1 py-0.5 flex items-center gap-0 overflow-x-auto scrollbar-none">
        <Globe className="h-3 w-3 text-muted-foreground mx-1 shrink-0" />
        {REGION_PRESETS.map(r => (
          <button
            key={r.id}
            onClick={() => map.flyTo(r.center, r.zoom, { duration: 1.5 })}
            className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase text-muted-foreground hover:text-foreground hover:bg-muted transition-colors shrink-0 whitespace-nowrap"
          >
            {r.label}
          </button>
        ))}
